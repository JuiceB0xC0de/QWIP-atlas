...
analyze_bouncer.py
------------------
Local-side analysis of bouncer_scores.json produced by extract_bouncer.py.
...
Cross-component comparison:
  * which component has the strongest bouncer signal per layer
  * which layer is the strongest bouncer-axis encoder overall

Usage:
  python analyze_bouncer.py --report bouncer_scores.json --top 20
"""
...
def cmd_analyze(args):
    ...
    print("=" * 100)
    print(f"  Top-{args.top} bouncer features per (layer, component) by F-stat")
    print("=" * 100)
    by_lc_rows = []
    ...
    print("\n=== Per-head bouncer ranking (multi-head components) ===")
    ph_rows = []
    for L in sorted(report.keys(), key=int):
        for comp, data in report[L].items():
    ...
    # 5. Overall winner
    print("\n=== Overall strongest bouncer-axis encoders ===")
    ...
    out_path = Path(args.outdir) / "bouncer_analysis.json"
...
def main():
    ...
    p.add_argument("--report", default="bouncer_scores.json",
